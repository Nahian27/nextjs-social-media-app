...
export default function NavigationCard() {
    ...
    return (
        <Card noPadding={true}>
            <div
                className="px-4 flex justify-between md:block shadow-md shadow-gray-500 md:shadow-none md:fixed md:h-screen md:border-r-2 md:pr-10 border-r-slate-800 pt-0 mt-0 z-40">
                ...
                <Link href="/search"
                      className={pathname === '/search' ? activeElementClasses : nonActiveElementClasses}>
                ...
                <Link href={'/profile/' + session?.user?.id}
                      className={pathname === `/profile/${session?.user?.id}` ? activeElementClasses : nonActiveElementClasses}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
                         stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round"
